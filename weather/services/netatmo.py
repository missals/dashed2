...
class NetAtmoAPI:
    ...
    def authenticate(self):
        ...
        try:
            response = requests.post("https://api.netatmo.com/oauth2/token", data=auth_payload)
    ...
    def check_and_refresh_token(self):
        ...
        # Check if we should have a valid token still. If not, refresh ...
        now = datetime.now()
    ...
    def refresh(self):
        ...
        try:
            response = requests.post("https://api.netatmo.com/oauth2/token", data=refresh_payload)
            response.raise_for_status()
            access_token = response.json()["access_token"]
            refresh_token = response.json()["refresh_token"]
            expires_in = response.json()["expires_in"]
    ...
    def current(self):
        ...
        for d in data['devices']:
            ...
            # Get main unit data
            device_data = d['dashboard_data']
            print(device_data['Temperature'])
            curtime = datetime.utcfromtimestamp(device_data['time_utc'])
            print(curtime)

            for m in d['modules']:
                print(m['module_name'])

                module_data = m['dashboard_data']
                print(module_data['Temperature'])
